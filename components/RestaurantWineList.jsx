...
const EMPTY_FILTERS = { color: "all", country: "all", region: "all", grape: "all" };
...
export default function RestaurantWineList({ listings }) {
  ...
  const filterLabels = { color: "Kleur", country: "Land", region: "Regio", grape: "Druif" };
  const chipLabel = (key, value) => (key === "color" ? COLOR_MAP[value].label : value);

  return (
    <>
      <p className="text-sm mt-7 mb-3" style={{ color: TEXT_MUTED }}>
        {rows.length} van {listings.length} wijnen
      </p>
      ...
      {panelOpen && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4 rounded-2xl mb-4" style={{ background: SURFACE, border: `1px solid ${BORDER}` }}>
          <Select label="Kleur" value={filters.color} onChange={(v) => setFilter("color", v)} options={availableColors.map((c) => ({ value: c, label: COLOR_MAP[c].label }))} />
